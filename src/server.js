...
// CAMBIO IMPORTANTE: Importamos getConnection globalmente para usarla en transacciones.
import { query, getConnection } from './db.js'; // Aseguramos que esta función use pool.execute
...
// ----------------------------------------------------------------------------------
// RUTA CORREGIDA: /api/attendees (Se mantiene la lógica que ya habías ajustado)
// ----------------------------------------------------------------------------------
/**
 * GET /api/attendees
...
app.post('/api/checkin', checkGlobalApiKey, async (req, res) => {
  // CORRECCIÓN: Usamos la función 'getConnection' importada globalmente.
  const conn = await getConnection().catch((err) => {
    console.error('[checkin] Error al obtener conexión:', err);
    return null;
  });
  if (!conn) {
  ...
  try {
    ...
    try { await conn.rollback(); } catch (rollbackErr) { console.error('Error al hacer rollback:', rollbackErr); }
    res.status(500).json({ ok: false, error: 'Error interno' });
  } finally {
    try { conn?.release(); } catch (releaseErr) { console.error('Error al liberar conexión:', releaseErr); }
  }
});
// ----------------------------------------------------------------------------------
